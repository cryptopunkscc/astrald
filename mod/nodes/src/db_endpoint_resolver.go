...
func (r *DBEndpointResolver) ResolveEndpoints(ctx *astral.Context, nodeID *astral.Identity) (<-chan *nodes.ResolvedEndpoint, error) {
	...
	go func() {
		...
		for _, row := range rows {
			...
			var re *nodes.ResolvedEndpoint
			if row.ExpiresAt != nil {
				remaining := time.Until(*row.ExpiresAt)
				if remaining > 0 {
					re = nodes.NewResolvedEndpoint(e, remaining)
				} else {
					continue
				}
			} else {
				re = nodes.NewResolvedEndpoint(e)
			}
