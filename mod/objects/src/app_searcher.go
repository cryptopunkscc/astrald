...
func NewAppSearcher(mod *Module, id *astral.Identity) *AppSearcher {
	return &AppSearcher{
		mod:     mod,
		id:      id,
...
func (s *AppSearcher) SourceIdentity() *astral.Identity { return s.id }

func (s *AppSearcher) SearchObject(ctx *astral.Context, q objects.SearchQuery) (<-chan *objects.SearchResult, error) {
	...
	if in == nil {
		...
		return nil, errors.New("app search returned no stream")
	}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				s.log.Errorv(1, "app searcher: %v", err)
				return
			}
			if !ok {
				break
		...
		if errPtr != nil && *errPtr != nil {
			s.log.Errorv(1, "app searcher: %v", *errPtr)
		}
	}()
