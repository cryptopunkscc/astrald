...
// Run binds the configured port, accepts connections in a loop, and dispatches each to onAccept in its own goroutine.
// If onAccept signals stopListener the server closes itself.
// Returns nil on clean shutdown (context cancelled or Close called); returns an error only on unexpected failures.
