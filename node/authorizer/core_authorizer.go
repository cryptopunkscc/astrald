...
var _ AuthSet = &CoreAuthorizer{}

type CoreAuthorizer struct {
	authorizers sig.Set[Authorizer]
	log         *log.Logger
}
...
func (auth *CoreAuthorizer) Add(authorizer Authorizer) error {
	return auth.authorizers.Add(authorizer)
}

func (auth *CoreAuthorizer) Remove(authorizer Authorizer) error {
