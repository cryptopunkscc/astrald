...
// (as opposed to a compile-time prototype or an empty/heterogeneous slot).
func isRuntimeBlueprintType(typeName string) bool {
	return typeName != "" && DefaultBlueprints().GetBlueprint(typeName) != nil
}
...
// settable *RuntimeObject slot. Returns total bytes consumed.
func readRuntimeBlueprintPtr(r io.Reader, typeName string, dst reflect.Value) (int64, error) {
	var nilFlag uint8
...
// readRuntimeBlueprintValue decodes a single RuntimeObject value (no nil-flag, no pointer)
...
// See readRuntimeBlueprintPtr for the nil-flag handling.
func readRuntimeBlueprintMapValue(r io.Reader, typeName string) (Object, int64, error) {
	var nilFlag uint8
	err := binary.Read(r, ByteOrder, &nilFlag)
	if err != nil {
		return nil, 0, err
