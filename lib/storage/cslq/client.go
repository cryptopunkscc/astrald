...
func NewClient(target id.Identity) *Client {
	return &Client{target: target, port: api.Port}
}
...
func (s Client) Put(b []byte, opts *storage.CreateOpts) (i data.ID, err error) {
	conn, err := s.query(api.Put, "[l]c v", b, opts)
	if err != nil {
		return
	}
	defer conn.Close()
