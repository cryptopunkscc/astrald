...
func (set *LinkSet) Add(l net.Link) (node.ActiveLink, error) {
	set.mu.Lock()
	defer set.mu.Unlock()
...
func (set *LinkSet) Find(id int) (node.ActiveLink, error) {
	set.mu.Lock()
	defer set.mu.Unlock()
...
func (set *LinkSet) All() []node.ActiveLink {
	set.mu.Lock()
	defer set.mu.Unlock()

	links := make([]node.ActiveLink, 0, len(set.links))
...
func (set *LinkSet) ByRemoteIdentity(identity id.Identity) node.LinkSet {
	set.mu.Lock()
	defer set.mu.Unlock()
...
func (set *LinkSet) ByLocalIdentity(identity id.Identity) node.LinkSet {
	set.mu.Lock()
	defer set.mu.Unlock()
